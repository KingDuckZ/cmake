...
{
  const char* ext = ".vcxproj";
  if(static_cast<cmGlobalVisualStudioGenerator*>(this->GlobalGenerator)
     ->TargetIsDOnly(*this->Target))
    {
    ext = ".visualdproj";
    }
...
void cmVisualStudio10TargetGenerator::WriteDProj()
{
  //TODO: Refactor me!
  //this->WriteString("<?xml version=\"1.0\" encoding=\"utf-8\"?>\n",0);
  this->WriteString("<DProject>\n", 0);
  this->WriteString("<!-- Generated by CMake -->\n", 0);
  ...
  for(std::vector<std::string>::iterator cfg = configs->begin();
    ...
    // in outdir rather than objdir.
    this->WriteString(
        "<singleFileCompilation>2</singleFileCompilation>\n", 2);

    cmTarget::TargetType tt = this->Target->GetType();
    int target_type_id = 0;
    if(tt == cmTarget::EXECUTABLE)
      target_type_id = 0;
    else if(tt == cmTarget::STATIC_LIBRARY)
      target_type_id = 1;
    else if(tt == cmTarget::SHARED_LIBRARY)
      target_type_id = 2;

    this->WriteString("<lib>", 2);
    (*this->BuildFileStream) << target_type_id << "</lib>\n";

    const char* dc_id = this->Target->GetMakefile()
        ->GetDefinition("CMAKE_D_COMPILER_ID");
    int dc_iid = 0;
    if(strcmp(dc_id, "DigitalMars") == 0)
      dc_iid = 0;
    else if(strcmp(dc_id, "GNU") == 0)
      dc_iid = 1;
    else if(strcmp(dc_id, "LLVM") == 0)
      dc_iid = 2;
    this->WriteString("<compiler>", 2);
    (*this->BuildFileStream) << dc_iid << "</compiler>\n";
    this->WriteString("<otherDMD>1</otherDMD>\n", 2);
    this->WriteString("<program>", 2);
    (*this->BuildFileStream) << "&quot;" <<
    ...
    if(!impList.empty())
      ...
      for(std::vector<std::string>::iterator it = impList.begin();
          it != impList.end(); ++it)
        {
        (*this->BuildFileStream) << "&quot;" << *it << "&quot; ";
        }
      this->WriteString("</imppath>\n", 0);
      }
    ...
    if(!textImpList.empty())
      ...
      for(std::vector<std::string>::iterator it = textImpList.begin();
          it != textImpList.end(); ++it)
        {
        (*this->BuildFileStream) << "&quot;" << *it << "&quot; ";
        }
    ...
    const char* flags = this->Target->GetProperty("COMPILE_FLAGS");
    if(flags)
      {
    ...
    // libfiles evidently *cannot* contain quotes
    std::set<std::string> libpaths;
    std::set<std::string> libfiles;
    cmTarget::LinkLibraryVectorType libs = this->Target->GetOriginalLinkLibraries();
    for(cmTarget::LinkLibraryVectorType::iterator it = libs.begin();
        it != libs.end(); ++it)
     {
     cmTarget* tgt = this->GlobalGenerator->FindTarget(it->first, true);
     std::string path;
     if(tgt)
       {
     ...
     unsigned found = path.rfind("/");
     if(found != std::string::npos)
       {
       libpaths.insert(path.substr(0, found));
       libfiles.insert(path.substr(found+1));
       }
     else
       {
       //FIXME: Handle case where / couldn't be found
       }
     }

    this->WriteString("<libfiles>", 2);
    for(std::set<std::string>::iterator f = libfiles.begin();
        f != libfiles.end(); ++f)
      {
      (*this->BuildFileStream) << *f << " ";
      }
    (*this->BuildFileStream) << "</libfiles>\n";

    this->WriteString("<libpaths>", 2);
    for(std::set<std::string>::iterator p = libpaths.begin();
        p != libpaths.end(); ++p)
      {
      (*this->BuildFileStream) << "&quot;" << *p << "&quot; ";
      }
  ...
  for(std::vector<cmSourceFile*>::const_iterator
    ...
    {
    //cmSourceFileLocation loc = (*oi)->GetLocation();
    this->WriteString("<File path=\"", 2);
    //(*this->BuildFileStream)
    //<< loc.GetDirectory() << "/" << loc.GetName() << "\" />\n";
    (*this->BuildFileStream) << (*oi)->GetFullPath() << "\" />\n";
    }

  this->WriteString("</Folder>\n", 1);
  this->WriteString("</DProject>\n", 0);
}
...
void cmVisualStudio10TargetGenerator::WriteProjectReferences()
{
  ...
  for( OrderedTargetDependSet::const_iterator i = depends.begin();
    ...
    // skip D and fortran targets as they can not be processed by MSBuild
    // the only reference will be in the .sln file
    cmGlobalVisualStudioGenerator* globalGen =
        static_cast<cmGlobalVisualStudioGenerator*>(this->GlobalGenerator);
    if(globalGen->TargetIsFortranOnly(*dt) || globalGen->TargetIsDOnly(*dt))
      {
      continue;
      }
